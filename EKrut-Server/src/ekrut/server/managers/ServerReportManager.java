...
public class ServerReportManager {
	...
	public Report generateOrderReport(LocalDateTime date, String ekrutLocation, String area) {
		...
		for (Order order : shipmentOrders) {
			...
			shipmentTotalOrderInILS += order.getSumAmount();
		}
			...
			for (Order order : entry.getValue()) {
				...
				switch (order.getType()) {
				    case PICKUP:
				    	...
				    	locationOrdersData.add(6, locationOrdersData.get(4) + order.getSumAmount());
				    	break;
				    case REMOTE:
				    	locationOrdersData.add(3, locationOrdersData.get(2) + 1);
				    	locationOrdersData.add(7, locationOrdersData.get(5) + order.getSumAmount());
				    	break;
				    default:
				    	break;
				  }
