...
public class ServerReportManager extends AbstractServerManager<ReportRequest, ReportResponse> {
	 ...
	 *
	 * @param reportRequest the request containing the parameters to use for report
	 *                      retrieval
	 * @param user          the user requesting the report
	 * @return a response containing the retrieved report or an error result if the
	 *         report was not found
	 */
	public ReportResponse fetchReport(ReportRequest reportRequest, User user) {

		Report report = reportDAO.fetchReport(reportRequest.getDate(), reportRequest.getLocation(),
				reportRequest.getArea(), reportRequest.getReportType());
	...
	public Report generateOrderReport(LocalDateTime date, String area) {
		...
		// Create a new Map to hold all the orders at the given area by locations
		// without shipment orders
		...
		/*
		 * shipmentOrders will contain all orders of the shipment type. The ArrayList
		 * will be filtered to include only orders from customers in the specified area.
		 */
		ArrayList<Order> shipmentOrders = filterShipmentOrdersByUserArea(reportDAO.fetchAllMonthlyShipmentOrders(date),
				area);
		...
		for (Map.Entry<String, ArrayList<Order>> entry : areaOrdersByLocations.entrySet()) {
			// Process each list of orders: Adds to each item the number of times it was
			// ordered in the list
			topSellers = countItemQuantitiesByLocation(entry.getValue(), topSellers);

			int totalOrders = 0;
			int totalOrdersInILS = 0;
			// Calculate totalOrders and totalOrdersInILS for each order at the current
			// location
		...
		Report report = new Report(null, ReportType.ORDER, date, area, area, areaTotalOrders, areaTotalOrdersInILS,
				shipmentTotalOrders, shipmentTotalOrderInILS, orderReportData, topSellers);
		// Return the report
		return report;
	}
	...
	public Report generateCustomerReport(LocalDateTime date, String ekrutLocation, String area) {
		...
		ArrayList<String> allCustomersOrders = reportDAO.getAllCustomersOrdersByNameWithOutShipment(date,
				ekrutLocation);
		// Process the customer orders to count the number of orders made by each
		// customer
		Map<String, Integer> customersOrders = countOrdersByCustomer(allCustomersOrders);
		// Create a histogram of customer orders by dividing customers into categories
		// based on their order count
		...
		ArrayList<LocalDateTime> allCustomersOrdersByDate = reportDAO.getAllCustomersOrdersByDateWithOutShipment(date,
				ekrutLocation);
		// Process the customer orders by date to count the number of orders made by
		// each customer on each day
	 ...
	 * 
	 * @param thersholdBreaches a list of item names for which threshold breach have
	 *                          been triggered
	 ...
	 * @return a map of item names to lists containing the item quantity and
	 *         threshold alert count
	 */
	private Map<String, ArrayList<Integer>> createInventoryReportData(ArrayList<InventoryItem> allItemsInLocation,
	 ...
	 *
	 * @param customersOrders a map containing customers as keys and their number of
	 *                        orders categories as values
	 * @return a map containing keys for the number of orders (1, 2, 3, 4, 5, 6+)
	 *         and values for the number of customers with that number of orders
	 */
	private Map<String, Integer> createCustomersHistogram(Map<String, Integer> customersOrders) {
		...
		for (Map.Entry<String, Integer> entry : customersOrders.entrySet()) {
			...
			// Increment the appropriate value in the histogram map based on the number of
			// orders
			switch (numOrders) {
			case 1:
	...
	/**
	 * Processes a list of customer orders and creates a map of customers and their
	 * number of orders.
	 *
	 * @param allCustomersOrders a list of customer orders
	 * @return a map containing customers as keys and their number of orders as
	 *         values
	 */
	...
	/**
	 * Processes a list of customer order dates and returns a map of order counts by
	 * date.
	 *
	...
	/**
	 * This method processes a map of item quantities and generates a map of the top
	...
	/**
	 * This method processes a list of orders and generates a map of item
	 * quantities.
	 * 
	 ...
	 */
	private Map<String, Integer> countItemQuantitiesByLocation(ArrayList<Order> orders,
			Map<String, Integer> itemsQuantityInOrders) {
	...
	/**
	 * Processes a map of orders by location and returns a map of order data for
	 * each location.
	 *
