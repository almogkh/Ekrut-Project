...
/**
 * An abstract class that provides utility methods for managing shipments. This
 * version will have the ability to change according to system requirements in
 * the future.
 * 
 * @author Nir Betesh
 */
public abstract class ShipmentManagerUtils {
	...
	/**
	 * Calculates the flight time in hours for a shipment from the given warehouse
	 * to the given client address.
	 * 
	...
	/**
	 * Calculates the total delivery duration time in hours for a shipment to the
	...
	private static long deliveryDurationTimeInHours(String clientAddress) {
		return flightTimeInHours(null, clientAddress) + ShipmentLoadTimeInHours() + watingForDroneTimeInHours();
	}

	/**
	 * Calculates the estimated arrival time for a shipment given the current date
